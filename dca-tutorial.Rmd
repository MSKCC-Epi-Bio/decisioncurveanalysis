...
output:
  html_document:
    ...
    css: "https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.7.0/styles/agate.min.css"
params:
  language: r
editor_options:
  markdown:
...
knitr::opts_chunk$set(
  echo = TRUE, warning = FALSE, fig.height = 3, cache = TRUE,
...
add_tabset_codes <- function(chunk_label, eval = TRUE, echo = FALSE,
  ...
  res <-
    knitr::knit_child(
      text = unlist(knitr::knit_expand("assets/templates/tabset-template.Rmd")),
      quiet = TRUE,
...
tibble::tibble(
  ...
  gt::cols_label(
    ...
    stata = gt::html('<b><a href="dca-tutorial-stat.html"><font color="black">Stata</a></b>'),
...
There are various options for improving the look of the graph. First, regular graph options apply. In Stata, for instance, you can add lcolor, lwidth and lpattern as options. For instance, `dca cancer marker, xstop(0.25) lcolor(red green blue) lwidth(thick medium thin) lpattern(solid dash dot)` would have "Treat all" as a thick, red, solid line, "treat none" as a medium thickness, green dashed line and marker as a thin, blue, dashed line (note: this would not look much good, but is given for illustrative purposes). In R, Python, and SAS, similar customization options are available through their respective plotting systems.

```{r, dca_formatting, echo=FALSE, message = FALSE, results='asis'}
add_tabset_codes("dca_formatting")
```
...
```{r, dca_legend_off, echo=FALSE, message = FALSE, results='asis'}
add_tabset_codes("dca_legend_off")
```
...
```{r, dca_create_low_incidence, echo=FALSE, message = FALSE, results='asis'}
add_tabset_codes("dca_create_low_incidence")
```
...
```{r, dca_adjust_axes, echo=FALSE, message = FALSE, results='asis'}
add_tabset_codes("dca_adjust_axes")
```
...
```{r, dca_combined_smooth, echo=FALSE, message = FALSE, results='asis'}
add_tabset_codes("dca_combined_smooth")
```
...
1. **Joint Approach**: Treat patients who are either high risk or have a marker prediction above a threshold.
2. **Conditional Approach**: Treat high-risk patients immediately; for intermediate-risk patients, use the marker to decide; and don't treat low-risk patients.
...
As we can see from the decision curve, the different approaches to combining risk group information with the marker have varying clinical utility across different threshold probabilities.
