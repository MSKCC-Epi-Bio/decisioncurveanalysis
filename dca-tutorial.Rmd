...
In Stata, for instance, you can add lcolor, lwidth and lpattern as options. For instance, `dca cancer marker, xstop(0.25) lcolor(red green blue) lwidth(thick medium thin) lpattern(solid dash dot)` would have "Treat all" as a thick, red, solid line, "treat none" as a medium thickness, green dashed line and marker as a thin, blue, dashed line (note: this would not look much good, but is given for illustrative purposes). In R, Python, and SAS, similar customization options are available through their respective plotting systems.

![Decision curve plot showing formatting options](assets/images/dca_formatting_example.png)
